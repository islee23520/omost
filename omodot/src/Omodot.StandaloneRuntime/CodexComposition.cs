...
using Omodot.SkillMcp;
using Omodot.SkillsCore;
...
public sealed class CodexComposedOmoRuntime
{
    ...
    private static CodexComposedOmoRuntime Compose(
        IUlwHost host,
        ICodexConversationClient? client,
        global::Omodot.CodexAdapter.CodexAdapterRuntime? adapterRuntime)
    {
        ...
        var skillMcpSkillLikes = skills.Select(s => new SkillMcpSkillLike(
            ...
            s.McpConfig is not null
                ? new Omodot.SkillMcp.SkillMcpConfig(
                    s.McpConfig.Servers.ToDictionary(
                        kv => kv.Key,
                        kv => new Omodot.SkillMcp.SkillMcpServerConfig(kv.Value.Command, kv.Value.Args, kv.Value.Env)))
                : null)).ToList();
