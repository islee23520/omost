...
public sealed class CodexMcpBridgeExecutableTests
{
    ...
    {
        var bridgeDll = Path.Combine(AppContext.BaseDirectory, "Omodot.CodexMcpBridge.dll");
        var bridgeRuntimeConfig = Path.Combine(AppContext.BaseDirectory, "Omodot.CodexMcpBridge.runtimeconfig.json");
        ...
        Assert.Equal("omodot_codex", initialize.RootElement.GetProperty("result").GetProperty("serverInfo").GetProperty("name").GetString());
