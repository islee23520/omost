...
public sealed class CodexPluginAssetsTests
{
    ...
    public void CreateMarketplaceJson_UsesCodexMarketplaceShape()
    {
        ...
        Assert.Equal("omodot-codex-plugins", doc.RootElement.GetProperty("name").GetString());
        var plugin = doc.RootElement.GetProperty("plugins")[0];
        Assert.Equal("omodot", plugin.GetProperty("name").GetString());
        Assert.Equal("./plugins/omodot", plugin.GetProperty("source").GetString());
    ...
    public void CreatePluginJson_DeclaresMcpServersAndInterface()
    {
        ...
        Assert.Equal("omodot", doc.RootElement.GetProperty("name").GetString());
        Assert.Equal("./.mcp.json", doc.RootElement.GetProperty("mcpServers").GetString());
        Assert.Equal("Omodot", doc.RootElement.GetProperty("interface").GetProperty("displayName").GetString());
    }

    [Fact]
    public void CreateMcpJson_DeclaresOmodotCodexServer()
    {
        using var doc = JsonDocument.Parse(CodexPluginAssets.CreateMcpJson());
        var server = doc.RootElement.GetProperty("mcpServers").GetProperty("omodot-codex");
        Assert.Equal("dotnet", server.GetProperty("command").GetString());
        Assert.Equal("Omodot.CodexMcpBridge.dll", server.GetProperty("args")[0].GetString());
